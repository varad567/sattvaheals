...
const css = `
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .al-brand-name { font-family: 'Cormorant Garamond', serif; font-size: 22px; font-weight: 600; color: var(--pearl); letter-spacing: 5px; text-transform: uppercase; display: block; }
  .al-brand-name span { color: var(--gold); }
  .al-brand-sub { font-size: 9px; letter-spacing: 3px; color: var(--moon-dim); text-transform: uppercase; margin-top: 4px; display: block; }

  .al-moon-wrap { position: absolute; top: 42%; left: 50%; transform: translate(-50%, -50%); z-index: 2; }
  .al-moon-glow {
    width: 320px; height: 320px; border-radius: 50%;
    background: radial-gradient(circle at center, rgba(168,204,224,0.07) 0%, rgba(168,204,224,0.03) 40%, transparent 70%);
    animation: mPulse 5s ease-in-out infinite;
    display: flex; align-items: center; justify-content: center;
  }
  @keyframes mPulse { 0%,100%{transform:scale(1);opacity:0.7} 50%{transform:scale(1.06);opacity:1} }
  .al-moon-body {
    width: 120px; height: 120px; border-radius: 50%;
    background: radial-gradient(circle at 38% 35%, #E8F4FF 0%, #C8E0F0 25%, #A8CCE0 50%, #7AAEC8 75%, #4A82A0 100%);
    box-shadow: 0 0 40px rgba(168,204,224,0.2), 0 0 80px rgba(168,204,224,0.08), inset -12px -12px 24px rgba(74,130,160,0.4);
  }
  .al-moon-ring { position: absolute; inset: -40px; border-radius: 50%; border: 1px solid rgba(168,204,224,0.08); animation: spin 60s linear infinite; }
  .al-moon-ring-2 { position: absolute; inset: -70px; border-radius: 50%; border: 1px dashed rgba(226,194,125,0.06); animation: spin 90s linear infinite reverse; }
  @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }

  .al-copy { position: relative; z-index: 2; }
  .al-headline { font-family: 'Cormorant Garamond', serif; font-size: 36px; font-weight: 600; line-height: 1.2; color: var(--pearl); margin-bottom: 16px; }
  .al-headline em { font-style: italic; color: var(--gold); }
  .al-sub { font-size: 14px; line-height: 1.8; color: var(--pearl-dim); font-weight: 300; max-width: 340px; }
  .al-skt { font-family: 'Noto Serif Devanagari', serif; font-size: 13px; color: var(--gold); letter-spacing: 2px; margin-top: 20px; display: block; }

  .auth-right { background: var(--abyss); display: flex; align-items: center; justify-content: center; padding: 48px 64px; position: relative; }
  .auth-right::before { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse at 50% 30%, rgba(168,204,224,0.03) 0%, transparent 60%); pointer-events: none; }

  .auth-form-wrap { width: 100%; max-width: 420px; position: relative; z-index: 1; }

  .auth-tabs { display: flex; gap: 2px; margin-bottom: 40px; border-bottom: 1px solid rgba(168,204,224,0.08); }
  .auth-tab { padding: 12px 24px; background: transparent; border: none; font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 400; letter-spacing: 1.5px; text-transform: uppercase; color: var(--pearl-dim); cursor: pointer; transition: all 0.3s; border-bottom: 2px solid transparent; margin-bottom: -1px; }
  .auth-tab.active { color: var(--pearl); border-bottom-color: var(--gold); }

  .form-headline { font-family: 'Cormorant Garamond', serif; font-size: 32px; font-weight: 600; color: var(--pearl); margin-bottom: 8px; line-height: 1.2; }
  .form-headline em { font-style: italic; color: var(--gold); }
  .form-sub { font-size: 14px; color: var(--pearl-dim); margin-bottom: 36px; line-height: 1.6; font-weight: 300; }

  .btn-google { width: 100%; padding: 14px 20px; background: rgba(168,204,224,0.05); border: 1px solid rgba(168,204,224,0.15); border-radius: 2px; color: var(--pearl); font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 400; letter-spacing: 0.5px; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; justify-content: center; gap: 12px; margin-bottom: 24px; }
  .btn-google:hover { background: rgba(168,204,224,0.1); border-color: rgba(168,204,224,0.3); }
  .google-icon { width: 18px; height: 18px; flex-shrink: 0; }

  .auth-divider { display: flex; align-items: center; gap: 14px; margin-bottom: 24px; }
  ...
  .alert-success { background: rgba(110,203,160,0.08); border: 1px solid rgba(110,203,160,0.2); color: #6ECBA0; }

  .auth-switch { text-align: center; margin-top: 28px; font-size: 13px; color: var(--pearl-dim); }
  .auth-switch button { background: none; border: none; color: var(--moon); font-family: 'Outfit', sans-serif; font-size: 13px; cursor: pointer; transition: color 0.3s; text-decoration: underline; text-underline-offset: 3px; text-decoration-color: rgba(168,204,224,0.3); padding: 0; margin-left: 6px; }
  .auth-switch button:hover { color: var(--pearl); }

  .auth-privacy { text-align: center; margin-top: 20px; font-size: 11px; color: rgba(139,175,196,0.35); line-height: 1.6; letter-spacing: 0.3px; }
  .auth-privacy span { color: var(--moon-dim); }

  .forgot-btn { background: none; border: none; color: var(--moon-dim); font-size: 12px; cursor: pointer; letter-spacing: 1px; font-family: 'Outfit', sans-serif; transition: color 0.3s; }
  .forgot-btn:hover { color: var(--moon); }

  .auth-success { text-align: center; padding: 40px 20px; animation: fadeUp 0.7s ease both; }
  .success-moon { font-size: 52px; margin-bottom: 20px; display: block; animation: mPulse 3s ease-in-out infinite; }
  .success-title { font-family: 'Cormorant Garamond', serif; font-size: 32px; font-weight: 600; color: var(--pearl); margin-bottom: 12px; }
  ...
  .success-next { font-size: 12px; letter-spacing: 2px; color: var(--moon-dim); text-transform: uppercase; display: flex; align-items: center; justify-content: center; gap: 10px; }
  .success-next::before, .success-next::after { content: ''; width: 24px; height: 1px; background: var(--gold-dim); }
...
function OceanWaves() {
  return (
    <div style={{position:'absolute',bottom:0,left:0,right:0,height:'45%',overflow:'hidden',zIndex:1,pointerEvents:'none'}}>
      <svg viewBox="0 0 800 300" xmlns="http://www.w3.org/2000/svg"
        style={{position:'absolute',bottom:0,width:'100%',height:'100%'}} preserveAspectRatio="none">
        <defs>
          <style>{`
            ...
            @keyframes wave1{0%,100%{d:path("M0,180 C120,140 200,200 320,170 C440,140 520,190 640,165 C720,148 760,170 800,160 L800,300 L0,300 Z")}50%{d:path("M0,170 C100,200 220,145 340,180 C460,215 540,155 660,178 C740,194 780,165 800,170 L800,300 L0,300 Z")}}
            @keyframes wave2{0%,100%{d:path("M0,210 C140,175 240,230 380,200 C500,172 600,215 720,195 C760,185 785,198 800,192 L800,300 L0,300 Z")}50%{d:path("M0,200 C120,228 260,180 380,215 C490,245 590,195 720,210 C768,220 790,205 800,200 L800,300 L0,300 Z")}}
            @keyframes wave3{0%,100%{d:path("M0,235 C160,210 280,250 400,228 C520,206 620,242 740,225 C772,217 790,228 800,222 L800,300 L0,300 Z")}50%{d:path("M0,228 C140,248 270,215 400,238 C520,260 630,222 740,240 C775,250 792,235 800,230 L800,300 L0,300 Z")}}
            @keyframes wave4{0%,100%{d:path("M0,260 C180,244 300,268 440,252 C560,237 660,262 780,250 L800,300 L0,300 Z")}50%{d:path("M0,252 C160,266 300,245 440,260 C570,274 670,248 780,258 L800,300 L0,300 Z")}}
          `}</style>
        </defs>
        ...
        <ellipse cx="400" cy="268" rx="60" ry="8" fill="rgba(168,204,224,0.06)"/>
        <ellipse cx="400" cy="272" rx="32" ry="4" fill="rgba(226,194,125,0.04)"/>
      </svg>
    </div>
  );
}
          ...
          <svg style={{position:'absolute',inset:-160,width:440,height:440,top:-160,left:-160}} viewBox="-220 -220 440 440">
            {dots.map((d,i)=>(
              <circle key={i} cx={d.x} cy={d.y} r={d.bright?2:1.2}
                fill={d.bright?'#E2C27D':'#A8CCE0'} opacity={d.bright?0.7:0.35}/>
            ))}
...
function SignupForm({ onSuccess }) {
  ...
  const handleGoogleLogin = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: 'https://sattvaheals.in' }
    });
    if (error) { console.error(error); setAlertMsg(error.message); }
  };
...
function LoginForm({ onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false);
    if (error) { setAlertMsg(error.message); return; }
    onSuccess(data.user?.user_metadata?.name || '');
  };

  const handleForgotPassword = async () => {
    if (!form.email.includes('@')) { setErrors({...errors, email:'Enter your email first'}); return; }
    const { error } = await supabase.auth.resetPasswordForEmail(form.email, {
      redirectTo: 'https://sattvaheals.in/reset-password'
    });
    if (error) { setAlertMsg(error.message); return; }
    setAlertMsg('✓ Password reset email sent. Check your inbox.');
  };
      ...
      <div className="auth-wrap">
        <div className="auth-left">
          ...
          <div className="al-copy">
            ...
            <span className="al-skt">सत्त्वम् — The pure nature of mind</span>
          </div>
        </div>
